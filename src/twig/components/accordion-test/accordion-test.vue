...
<script>
    export default {
        ...
        methods: {
            toggleActive(b, e) {
                const panel = e.target.closest('[data-accordion-test-panel]');
                const panelSiblings = this.getSiblings(panel);
                const window = panel.querySelector('[data-accordion-test-window]');
                const content = panel.querySelector('[data-accordion-test-content]');
                ...
                if (this.single == 1) {
                    ...
                    panelSiblings
                        ...
                        .forEach((sibling) => {
                            const siblingWindow = sibling.querySelector(
                                '[data-accordion-test-window]'
                            );
