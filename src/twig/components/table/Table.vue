<template>
  ...
  >
    <label
      class="relative flex shrink-0 md:shrink mb-6 md:mb-0 text-[0] w-full max-w-sm"
    >
      Search
      <svg
    ...
    >
      ...
      <option
        v-text="'Interdisciplinary Studies'"
      ...
      <button
        ...
        :class="{
          '!text-indigo font-bold':
            this.filterTerm.some((t) => this.filterOptions.includes(t)) ==
            false,
        }"
        v-text="'All'"
...
export default {
  ...
  computed: {
    ...
    paginatedItems() {
      return this.inputFilteredItems.slice(
        this.pageIndexStart,
        this.pageIndexEnd
      );
    },
  ...
  methods: {
    ...
    renderDesc(item) {
      const creditHours = this._creditHours(
        item.crsno,
        item.minhrs,
        item.maxhrs
      );
