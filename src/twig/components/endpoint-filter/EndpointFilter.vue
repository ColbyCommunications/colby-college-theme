<template>
  <div class="endpoint-filter">
    ...
    <div class="grid grid-cols-9 gap-10 max-w-screen-2xl w-full my-0 px-5 mx-auto mt-14">
        ...
        <article class="article space-y-4">
          <div class="context w-full space-y-5">
            <div class="text-group--animated text-group text-group--animated">
              <div class="text-group__subheading font-extended font-bold text-12 leading-130 tracking-8 text-left text-azure uppercase" v-text="item.date" />
              <h2 class="text-group__heading font-extended font-bold text-24 md:text-18 leading-110 -tracking-3 text-left text-indigo mt-2" v-text="item.title" />
              <p class="text-group__p font-body font-normal text-12 md:text-12 leading-130 text-left text-indigo-800 mt-2 !opacity-100" v-text="item.time" />
            </div>
            <div class="button-group--animated button-group flex flex-wrap gap-4 shrink-0">
              <a class="btn group inline-flex flex-row items-center space-x-1.5 rounded border border-solid border-indigo-300 hover:border-indigo-500 font-body font-normal text-10 leading-130 !no-underline text-indigo bg-indigo-100 hover:bg-indigo-200 focus:bg-indigo-200 focus:outline focus:outline-2 focus:outline-canary outline-offset-[-1px] py-1 px-3 transition-all duration-200 ease-in-out !opacity-100 !translate-x-0" :href="item.url" target="_blank">
...
<script>
  ...
  export default {
    ...
    async mounted() {
      this.endpoint = 'https://events.colby.edu/live/json/events/group/Colby%20Arts%20EMS/group/Museum%20of%20Art/group/Art/group/Center%20for%20Arts%20and%20Humanities/group/Creative%20Writing/group/Music/group/Cinema%20Studies/group/Performance%2C%20Theater%2C%20%26%20Dance';
      this.filters = [
        {
          title: 'All',
